...
const TOOLS = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                question: {
                    type: "string",
                    description: "The prediction market question to estimate (e.g. 'Will the Fed cut rates by June 2026?')",
                },
                market_price: {
                    type: "number",
                ...
                category: {
                    ...
                    description: "Market category for improved estimation (optional). Supported: policy, crypto, fed, geopolitics, technology, markets, politics, economics, other",
                },
                context: {
                    type: "string",
                    description: "Additional context or background information to refine the estimate (optional)",
                },
            },
    ...
    },
    {
        ...
        description: "Kelly-optimal position sizing for a prediction market trade. Uses premium Kelly parameters (α=0.75, conf_exp=1.0) that produce 2.6x P&L lift over free defaults. Applies market filter rules (skip fed/ultra-low/short-duration, boost policy/tech/markets). Costs $0.08 via x402 micropayment (USDC on Base).",
        inputSchema: {
            type: "object",
            properties: {
                estimated_prob: {
                    type: "number",
                    description: "Your estimated probability of the event (0.0 to 1.0), typically from prediction_market_estimate",
                },
                market_price: {
                    type: "number",
                    description: "Current market price as a probability (0.0 to 1.0)",
                ...
                confidence: {
                    type: "number",
                    description: "Your confidence in the estimate (0.0 to 1.0), typically from prediction_market_estimate",
                },
                category: {
                    type: "string",
                    enum: [
                ...
                bankroll: {
                    type: "number",
                    description: "Total available capital in USD for kelly sizing (default: 200)",
                },
            },
    ...
    },
    {
        ...
        description: "Scan and rank multiple prediction markets by tradeable edge. Applies proprietary market filter (skip rules remove low-quality markets, boost rules amplify high-edge categories). Returns ranked opportunities with Kelly sizing and filter status. Costs $0.10 via x402 micropayment (USDC on Base).",
        inputSchema: {
            type: "object",
            properties: {
                markets: {
    ...
    },
    {
        ...
        description: "Full prediction market analysis pipeline — Kalshalyst probability estimation + Xpulse social signal detection + ensemble weighting (0.75/0.25 Kalshalyst/Xpulse) + Kelly position sizing + market filter. The complete stack that produces 0.893 trading score. Costs $0.15 via x402 micropayment (USDC on Base).",
        inputSchema: {
            type: "object",
            properties: {
                question: {
                ...
                xpulse_posts: {
                    type: "array",
                    items: { type: "string" },
                    description: "X/Twitter posts related to the question for social signal analysis (optional)",
                },
                context: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Get information about the Prediction Market Analyst service — available endpoints, pricing, methodology overview, and how x402 payment works. Free, no payment required.",
        inputSchema: {
            type: "object",
            properties: {},
        },
    ...
    switch (name) {
        ...
        case "prediction_market_ensemble": {
            const result = await callAPI("/api/ensemble", "POST", {
                ...
                market_price: args.market_price,
                category: args.category,
                xpulse_posts: args.xpulse_posts || [],
                context: args.context,
            });
            return { content: [{ type: "text", text: result }] };
        }
